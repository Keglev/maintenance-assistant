...
| **Techniker** (shop-floor technician) | Search; receives full technical answers including repair steps, with citations to verify against, and machine filtering to keep results relevant. |
| **Schichtleiter** (shift supervisor) | All of the above, plus sole write access: creates and uploads protocols and sees their processing status. Write access is restricted here deliberately, to protect knowledge-base quality. |
| **Admin** (IT role only) | Manages users and role assignments in Keycloak without code changes. |
