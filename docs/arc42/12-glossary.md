...
| **Four eyes** | Author, corrector and approver are three different people. Enforced by the ROLE SPLIT and nothing else: nobody holds two of the three jobs (2026-08-13). A ledger check compared usernames at runtime until 2026-08-15; it became unreachable when the split landed, fired only on data written before it, and was replaced by `RoleMatrixIT` — a test that measures who may reach each endpoint and fails the build if any role gains both a corpus-write authority and the approve authority. |
