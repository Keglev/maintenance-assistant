...
| **Schichtleiter** (shift supervisor) | All of the above, plus writing and **correcting** protocols (v1.2) with a mandatory reason and a forced re-index. May not approve: the corrector is never the approver. |
| **Admin** (IT role only, plus moderation since v1.1) | Manages users and roles in Keycloak; reviews the corpus, archives protocols, and since v1.2 **approves** them. May not approve a protocol they filed or last corrected — four eyes, enforced on the act rather than by the role. |
