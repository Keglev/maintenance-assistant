...
| Data residency | Every component is self-hosted on one EU VPS; the only outbound call is to an EU-hosted, OpenAI-compatible LLM endpoint. | [ADR-002](../adr/ADR-002-eu-hosted-llm-provider.md) |
...
| Cost control | Three layers: provider-side prepaid/spending limits, application-side per-user rate limiting + global daily budget + capped answer length + query cache, and daily token-usage logging. Exhaustion produces a friendly message, never an error page. | NFR-7 |
