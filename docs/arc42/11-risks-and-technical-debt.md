...
| R-2 | **The fixer is not the documenter.** ~~Write access is restricted to the Schichtleiter, so knowledge capture depends on the shift handover.~~ **RETIRED 2026-08-11 by decision 3, applied to the UI 2026-08-28:** the Techniker writes, so the person at the machine is the person who files. | Protocols may be thin or lag behind the actual repair. | Was accepted deliberately in exchange for knowledge-base quality; the exchange is no longer needed. Quality is now held by the four-eyes rule instead — a writer may never correct their own protocol, so a second person still reads it. |
